    ...
    >
      ...
      <View style={c.cardTop}>
        <View
          style={[
            c.typeIcon,
            { backgroundColor: isWish ? '#FFF7ED' : `${colorss.primary}12` },
          ]}
        >
          {isWish ? (
        ...
        <View style={c.cardMeta}>
          <Text style={c.cardType}>
            {isWish ? 'Hope Wish (Video)' : 'Premium Call'}
          </Text>
      ...
      >
        <FastImage
          source={
            peerInfo?.avatarUrl ? { uri: peerInfo.avatarUrl } : IC_PROFILE
          }
          style={c.peerAvatar}
          resizeMode={FastImage.resizeMode.cover}
        />
        <View style={{ flex: 1 }}>
          <Text style={c.peerLabel}>
            {role === 'booked' ? 'Booking with' : 'Booking from'}
          </Text>
          <Text style={c.peerName} numberOfLines={1}>
            {peerInfo?.name || 'Hopenity user'}
          </Text>
        </View>
      ...
      {role === 'received' && booking.callerNote ? (
        <View style={c.noteBox}>
          <Text style={c.noteText} numberOfLines={2}>
            {booking.callerNote}
          </Text>
        </View>
      ) : null}
        ...
        <View style={c.actionsRow}>
          ...
          >
            {isRejecting ? (
              ...
              <>
                ...
                <Text style={c.rejectText}>Decline</Text>
              </>
            )}
          </TouchableOpacity>
          <TouchableOpacity
          ...
          >
            {isAccepting ? (
              ...
              <>
                <Check size={14} color={colorss.white} />
                <Text style={c.acceptText}>Accept</Text>
              </>
            )}
          </TouchableOpacity>
        </View>
      ...
      {showSendVideo && (
        ...
        >
          {isUploading ? (
            ...
            <>
              <Video size={15} color={colorss.white} />
              <Text style={c.sendVideoText}>Send Video Response</Text>
            </>
          )}
        </TouchableOpacity>
      )}
...
export default function MyBookingsScreen({ navigation }: Props) {
  ...
  const [uploadingBookingId, setUploadingBookingId] = useState<number | null>(
    null,
  );
  ...
  const getPeerInfo = useCallback(
    (peerId: string): PeerInfo | undefined => {
      const fromConvo = conversations.find(cv => cv.peerUserId === peerId);
      if (fromConvo)
        return { name: fromConvo.name, avatarUrl: fromConvo.avatarUrl ?? null };
  ...
  useEffect(() => {
    if (!token) return;
    const allPeerIds = [
      ...bookedList.map(b => b.calleeId),
      ...receivedList.map(b => b.callerId),
    ];
    const missing = Array.from(
      new Set(allPeerIds.filter(id => !!id && !getPeerInfo(id))),
    );
    if (missing.length === 0) return;
    let cancelled = false;
    Promise.all(missing.map(id => getOrCreatePeerChatWithInfo(id, token))).then(
      results => {
        if (cancelled) return;
        setPeerInfoMap(prev => {
          const next = { ...prev };
          missing.forEach((id, i) => {
            ...
            if (info)
              next[id] = { name: info.peerName, avatarUrl: info.peerAvatarUrl };
          });
          return next;
        });
      },
  ...
  const resolveChatInfo = useCallback(
    async (booking: CallBooking, role: Tab) => {
  ...
  const openChat = useCallback(
    async (booking: CallBooking, role: Tab) => {
      ...
      try {
        ...
        const peerAvatar = (info as any).peerAvatarUrl as
          | string
          | null
          | undefined;
  ...
  const handleSendVideoResponse = useCallback(
    async (booking: CallBooking) => {
        ...
        async response => {
          ...
          try {
            ...
            if (!info) {
              Alert.alert(
                'Error',
                'Could not find the chat thread. Please try again.',
              );
              return;
            }
    ...
    <View style={[s.safe, { paddingTop: insets.top }]}>
      ...
      {currentLoading ? (
        ...
        <FlatList
          ...
          renderItem={({ item }) => {
            const peerId =
              activeTab === 'booked' ? item.calleeId : item.callerId;
            return (
              <BookingCard
                booking={item}
                role={activeTab}
