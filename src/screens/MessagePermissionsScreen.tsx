...
import { getMessageOpenTo, setMessageOpenTo, type MessageOpenTo } from '../services/chatPrefs';
...
const OPTIONS: { value: MessageOpenTo; label: string; sub: string }[] = [
  { value: 'everyone', label: 'Everyone', sub: 'Anyone on HopeChat can send you messages.' },
  { value: 'contacts', label: 'My contacts only', sub: 'Only people whose chats you have accepted can message you.' },
];
...
const MessagePermissionsScreen = ({ navigation }: { navigation: any }) => {
  ...
  const styles = useMemo(() => StyleSheet.create({
    ...
    content: { padding: 20 },
    sectionTitle: {
      fontSize: 13, fontWeight: '600', color: colorss.textSecondary,
    ...
    radio: {
      ...
      borderColor: colorss.border, justifyContent: 'center', alignItems: 'center', marginTop: 1,
    },
    radioSelected: { borderColor: colorss.accent },
    radioDot: { width: 10, height: 10, borderRadius: 5, backgroundColor: colorss.accent },
    optionText: { flex: 1 },
    optionLabel: { color: colorss.textPrimary, fontSize: 16, fontWeight: '500', marginBottom: 3 },
    optionSub: { color: colorss.textSecondary, fontSize: 13, lineHeight: 18 },
    hint: { color: colorss.textSecondary, fontSize: 12, lineHeight: 18, marginTop: 20 },
  }), [colorss]);

  const handleSelect = (value: MessageOpenTo) => {
    setSelected(value);
    setMessageOpenTo(value);
    if (token) patchUserSettings({ messageOpenTo: value }, token);
  };
  ...
  return (
    <SafeAreaView style={styles.safeArea}>
      ...
      <View style={styles.content}>
        ...
        {OPTIONS.map(opt => (
          <TouchableOpacity key={opt.value} style={styles.option} onPress={() => handleSelect(opt.value)} activeOpacity={0.7}>
        ...
        <Text style={styles.hint}>
          Changing this setting affects who can start a new conversation with you. Existing accepted chats are not affected.
        </Text>
      </View>
    </SafeAreaView>
  );
