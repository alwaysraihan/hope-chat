...
function notifMessage(n: NotifItem): string {
  ...
  switch (n.type) {
    ...
    case 'PAGE_FOLLOW':
      return `${name} followed your page.`;
    case 'FRIEND_REQUEST':
    ...
    case 'BLOOD_REQUEST':
      return `${name} posted a blood request.`;
    case 'DONATION_REQUEST':
      return `${name} made a donation request.`;
    case 'DONATION_ACCEPTED':
      return `Your donation was accepted by ${name}.`;
    case 'DONATION_REJECTED':
      return `Your donation was declined by ${name}.`;
    case 'POST_UPLOAD_QUEUED':
      return 'Your post is being uploaded.';
    case 'POST_PUBLISHED':
      return 'Your post has been published.';
    case 'POST_BLOCKED':
      return 'Your post was removed for violating community guidelines.';
    case 'RECHARGE_APPROVED':
      return 'Your recharge request was approved.';
    case 'RECHARGE_DECLINED':
      return 'Your recharge request was declined.';
    ...
    case 'VERIFICATION_REJECTED':
      return 'Your verification request was rejected.';
    case 'DEVICE_APPROVAL_REQUEST':
      return 'A new device is requesting access to your account.';
    case 'AD_CAMPAIGN_ACTIVITY':
      return 'Your ad campaign has activity.';
    case 'AD_CAMPAIGN_COMPLETED':
      return 'Your ad campaign has completed.';
    case 'CONTEST_APPROVED':
      return 'Your contest entry was approved.';
    case 'CONTEST_REJECTED':
      return 'Your contest entry was rejected.';
    default:
...
async function markRead(id: number, token: string | null): Promise<void> {
  await apiFetch(`${API_BASE_URL}/api/v1/notifications/${id}/read`, token, {
    method: 'PATCH',
    body: '{}',
  });
}
...
async function markAllRead(token: string | null): Promise<void> {
  await apiFetch(`${API_BASE_URL}/api/v1/notifications/read-all`, token, {
    method: 'PATCH',
    body: '{}',
  });
}
    ...
    ({ item }: { item: NotifItem }) => (
      ...
      >
        ...
        <View style={styles.itemBody}>
          <Text
            style={[styles.itemMsg, !item.is_read && styles.itemMsgUnread]}
            numberOfLines={2}
          >
            {notifMessage(item)}
          </Text>
          <Text
            style={[styles.itemTime, !item.is_read && styles.itemTimeUnread]}
          >
